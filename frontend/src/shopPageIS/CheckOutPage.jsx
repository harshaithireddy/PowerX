...
const CheckoutPage = () => {
    ...
    useEffect(() => {
        const fetchCartItems = async () => {
            ...
            try {
                const response = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/cart/${userEmail}`);
                const cartData = response.data;

                const productsWithDetails = await Promise.all(
                    cartData.map(async (item) => {
                        const productResponse = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/prod-getid/${item.productId}`);
                        return {
                            ...item,
    ...
    const handleConfirmOrder = async () => {
        ...
        try {
            await axios.post(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/checkout`, orderData);
