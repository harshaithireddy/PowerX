...
const ShopPage = () => {
    ...
    useEffect(() => {
        const fetchProductsAndCart = async () => {
            try {
                // Fetch products
                const productsResponse = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/prod-get`);
                setProducts(productsResponse.data);

                // Fetch cart items for the logged-in user
                const cartItemsResponse = await axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/cart/${userEmail}`);
    ...
    const handleAddToCart = async (productId, e) => {
        ...
        try {
            await axios.patch(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/cart-add`, { email: userEmail, productId, quantity: 1 });
                    ...
                    getFilteredProducts().map((prod) => (
                        <div key={prod._id} className="productsSub" onClick={() => handleProductClick(prod.name)}>
                            ...
                            {cartQuantities[prod._id] > 0 ? (
                                <button className="addToCartButtonSP" onClick={handleGoToCart}>Go to Cart</button>
                            ) : (
                                <button className="addToCartButtonSP" onClick={(e) => handleAddToCart(prod._id, e)}>Add to Cart</button>
                            )}
                        </div>
                    ))
                )}
