...
const CartPage = () => {
    ...
    const fetchCartItems = async () => {
      ...
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/cart/${userEmail}`
        );
        ...
        const productsWithDetails = await Promise.all(
          cartData.map(async (item) => {
            const productResponse = await axios.get(
              `${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/prod-getid/${item.productId}`
            );
            return {
              ...item,
  ...
  const handleQuantityChange = async (itemId, change) => {
    ...
    if (updatedQuantity < 1) {
      ...
      try {
        await axios.patch(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/cart-add`, {
          email: userEmail,
  ...
  const handleRemoveItem = async (itemId) => {
    try {
      ...
      await axios.delete(`${import.meta.env.VITE_REACT_APP_BACKEND_BASEURL}/cart-remove`, {
